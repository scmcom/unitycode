...
//================================================
// recursive subdivision of a graphical mesh into increasingly smaller inscribed triangles
//================================================
// generates a sphere at runtime
// args passed from PCGSphereTester.cs

public static class PCGSphereCreator {
	...
	public static Mesh Create(int subdivisions, float radius){
		//debugging, output text to console log
		if(subdivisions < 0){
			subdivisions = 0;
			Debug.LogWarning("subdivisions increased to min 0.");
		}
		//debugging, output text to console log
		else if(subdivisions > 6){
			subdivisions = 6;
			Debug.LogWarning("subdivisions decreased to max 6.");
		}
		...
		CreatePCG(vertices, triangles, resolution);
	...
	private static void CreatePCG(Vector3[] vertices, int[] triangles, int resolution){
		...
		for (int i=0; i<=resolution; i++){
			float workdone = (float)i / resolution;
			Vector3 from, to;
			vertices[v++] = to = Vector3.Lerp (Vector3.down, Vector3.forward, workdone);
			for(int d = 0; d<4; d++){
				from = to;
				to = Vector3.Lerp (Vector3.down, directions[d], workdone);
				t = CreateLower(i,v,vBottom,t,triangles);
		...
		for (int i=resolution -1; i>=1; i--){
			float workdone = (float)i/resolution;
			Vector3 from, to;
			vertices[v++] = to = Vector3.Lerp(Vector3.up, Vector3.forward, workdone);
			for(int d=0; d<4; d++){
				from = to;
				to = Vector3.Lerp(Vector3.up, directions[d], workdone);
				t = CreateUpper(i,v,vBottom,t,triangles);
				v = CreateVertexLine(from,to,i,v,vertices);
				vBottom += i + 1;
			}
			vBottom = v - 1 - i * 4;
	...
	//create lower strip
	private static int CreateLower(int steps, int vTop, int vBottom, int t, int[] triangles){
		for (int i=1; i< steps; i++){
			triangles[t++] = vBottom;
			triangles[t++] = vTop -1;
			triangles[t++] = vTop;
	...
	//create upper strip
	private static int CreateUpper(int steps, int vTop, int vBottom, int t, int[] triangles){
		triangles[t++] = vBottom;
		triangles[t++] = vTop -1;
		triangles[t++] = ++vBottom;
		for(int i=1; i<=steps; i++){

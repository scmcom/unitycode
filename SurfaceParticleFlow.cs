...
public class SurfaceParticleFlow : MonoBehaviour 
{
	...
	private void PositionParticles()
	{
		...
		float amplitude = 
			surface.damping ? surface.strength / surface.frequency : surface.strength;
		for(int i=0; i<particles.Length; i++)
		{
			Vector3 position = particles[i].position;
			Vector3 point = q * new Vector3(position.x, position.y) + surface.offset;
